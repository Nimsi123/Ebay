...
- ``eBayScraper/ItemOrganization`` holds scripts that dictate the state of values at run-time. Such values are the scraping client's rotation, the data structure that stores and graphs the item data, and the class that organizes the flow of events.
